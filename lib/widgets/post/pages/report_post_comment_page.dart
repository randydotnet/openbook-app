...
class OBReportPostCommentPage extends StatefulWidget {
  final Post reportedPost;
  final PostComment reportedPostComment;
  final ReportCategory reportedCategory;

  const OBReportPostCommentPage(
      {Key key, @required this.reportedPost, @required this.reportedPostComment, @required this.reportedCategory})
      : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return OBReportPostCommentPageState();
  }
}

class OBReportPostCommentPageState extends State<OBReportPostCommentPage> {
    ...
    return Padding(
      ...
      child: Column(
        children: <Widget>[
          ...
          const OBText(
              'If you report someones comment, we do not tell them who reported it.'),
          const SizedBox(
            height: 40,
          ),
          _getCommentField(),
  ...
  void _onConfirm() async {
    ...
    try {
      await _userService.createPostCommentReport(
          postUuid: widget.reportedPost.uuid,
          postCommentId: widget.reportedPostComment.id,
